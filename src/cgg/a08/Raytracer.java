...
public class Raytracer {
	...
	public Raytracer(Group scene, int width, int height, int sf, Lochkamera lk, int core) {
	...
	public Image raytrace() {
		for (int x = 0; x < width; x++) {
			if(x % 10 == 0) System.out.println(x + " WIDTH VON " + width);
	...
	public Color getColor(double x, double y, Lochkamera lk) {
		Ray r = lk.generateRay(x + (core * width), y);
