...
public class Random extends java.util.Random {
    ...
    public long nextLong(final long n) {
        if (n <= 0)
            throw new IllegalArgumentException(
                    "illegal bound " + n + " (must be positive)");
        long t = staffordMix13(x += PHI);
        final long nMinus1 = n - 1;
        if ((n & nMinus1) == 0)
            return t & nMinus1;
        for (long u = t >>> 1; u + nMinus1
                - (t = u % n) < 0; u = staffordMix13(x += PHI) >>> 1)
            ;
        return t;
    }
    ...
    public double nextDouble() {
        return Double
                .longBitsToDouble(staffordMix13(x += PHI) >>> 12 | 0x3FFL << 52)
                - 1.0;
    }
    ...
    public float nextFloat() {
        return Float
                .intBitsToFloat(
                        staffordMix4Upper32(x += PHI) >>> 41 | 0x3F8 << 20)
                - 1.0f;
    }
