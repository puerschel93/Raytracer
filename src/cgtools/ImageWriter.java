...
public class ImageWriter {
    ...
    public static void write(String filename, double[] data, int width,
            int height) {
        try {
            ...
            System.out
                    .println(String
                            .format("Something went wrong writing: %s: %s",
                                    filename, error));
            System.exit(1);
        }
    }
    ...
    private static void write(String filename, double[] data, int width,
            int height, boolean linear) throws IOException {
        ...
        ComponentColorModel ccm = new ComponentColorModel(
                ColorSpace.getInstance(cs), false, false,
                ComponentColorModel.OPAQUE, DataBuffer.TYPE_USHORT);

        WritableRaster raster = Raster
                .createBandedRaster(DataBuffer.TYPE_USHORT, width, height, 3,
                        null);
        ...
        for (int y = 0; y != height; y++) {
            for (int x = 0; x != width; x++) {
                int i = (width * y + x) * 3;
                int[] rgb = { (int) (clamp(data[i + 0]) * 65535.0),
